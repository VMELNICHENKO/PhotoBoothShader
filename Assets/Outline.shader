Shader "CSShop/PhotoBooth/EdgeLine"
{
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float3 normal = SAMPLE_TEXTURE2D(_CameraNormalsTexture, sampler_CameraNormalsTexture, i.texcoord).rgb;
				// float3 viewNormal = normal0 * 2 - 1;
				float NdotL = 1 - dot(normal * 2 - 1, -i.viewSpaceDir);

                // Считаем Dot Product для нормали и направления к источнику света
                // float NdotL = dot(i.viewSpaceDir, normal);

                // Cчитаем интенсивность света на поверхности
                // Если поверхность повернута к источнику света (NdotL > 0),
                // то она полностью освещена.
                // В противном случае учитываем Shadow Strength для затенени
                 NdotL = 1 + clamp(floor(NdotL), -1, 0);
                 float lightIntensity = NdotL > 0 ? 1 : 0.3;
